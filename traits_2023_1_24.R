...
# Random forest VIP of the revolved 12 months model
#
set.seed(1955)
...
rev_12mos_5var_prune = prune.rpart(rev_12mos.tree, cp=cp.choice)
#
prp(rev_12mos_5var_prune, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA,  under = T, tweak = 1.1, varlen = 0, faclen = 0, Margin = 0.0, digits = -2, main='Revolved during the past 12 months: Classification tree')#faclet=0 avoids abbreviations, tweak for char size. I remove extra = 100 to remove percentage of observations.
...
# Random forest VIP [Synthetic dataset]
#
set.seed(1955)
