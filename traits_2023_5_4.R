...
# The paper itself contains only classification trees (not VIP) in the Appendix
# Final version uses traits scaled 1 to 5 (not the standardized to 0 mean and unit variance)
#
...
setwd("~/Papers/traits/traits_R_code_data")
dir()

### Reading RDS dataset
i2.df = readRDS("traits_public_2023_5_4.rds")
dim(i2.df)
...
#i7.df = subset(i6.df, select = c(age,  gender_factor, race_factor, ethnicity_factor, edu_factor,  marital_factor, income_hh, work_factor, hh_size,  homeowner_factor, urban_factor, id, extroversion_std, agreeableness_std, conscientiousness_std, neuroticism_std, openness_std, revolve_12m_factor,  revolve_1m_factor, cc_adopt,  unbanked))
# switching traits to be 1:5 instead of normalized
i7.df = subset(i6.df, select = c(age,  gender_factor, race_factor, ethnicity_factor, edu_factor,  marital_factor, income_hh, work_factor, hh_size,  homeowner_factor, urban_factor, id, extroversion, agreeableness, conscientiousness, neuroticism, openness, revolve_12m_factor,  revolve_1m_factor, cc_adopt,  unbanked))
...
colnames(i7.df)[colnames(i7.df)== "extroversion"] = "Extroversion" #=> switching traits to be 1:5 instead of normalized
#colnames(i7.df)[colnames(i7.df)== "extroversion_std"] = "Extroversion"
#colnames(i7.df)[colnames(i7.df)== "agreeableness_std"] = "Agreeableness"
colnames(i7.df)[colnames(i7.df)== "agreeableness"] = "Agreeableness"
#colnames(i7.df)[colnames(i7.df)== "conscientiousness_std"] = "Conscientiousness"
colnames(i7.df)[colnames(i7.df)== "conscientiousness"] = "Conscientiousness"
#colnames(i7.df)[colnames(i7.df)== "neuroticism_std"] = "Neuroticism"
colnames(i7.df)[colnames(i7.df)== "neuroticism"] = "Neuroticism"
#colnames(i7.df)[colnames(i7.df)== "openness_std"] = "Openness"
colnames(i7.df)[colnames(i7.df)== "openness"] = "Openness"
# summarize traits after renaming
summary(i7.df$Extroversion)
summary(i7.df$Agreeableness)
summary(i7.df$Conscientiousness)
summary(i7.df$Neuroticism)
summary(i7.df$Openness)
...
dim(rev1.df)
# Verify that non credit card adopters are excluded
table(rev1.df$cc_adopt, exclude = F)

# restricting to actually used variables
rev2.df = rev1.df
table(rev2.df$unbanked)# too few => remove
#
rev3.df = subset(rev2.df, select = c(Age,  Gender, Race, Ethnicity, Education,  Marital, HH_income, Work, HH_size,  Homeowner, Urbanicity, id, Extroversion, Agreeableness, Conscientiousness, Neuroticism, Openness, revolve_12m_factor,  revolve_1m_factor))
#
dim(rev3.df)
...
# model 1 has all variables. [Not used in the paper]
...
# [Model 1 is not used in the paper]
...
#
set.seed(1955) # [Model 1 is not used in the paper]
...
#
prp(rev_1m_5var_prune, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA,  under = T, tweak = 1.2, varlen = 0, faclen = 0, Margin = 0.0, digits = -2, main='Revolved during last month: Classification tree', extra=0)#faclet=0 avoids abbreviations, tweak for char size. extra=1,2,.. provides more info such as number of observations.
...
cc1.df = subset(i7.df, select = c(cc_adopt, Age,  Gender, Race, Ethnicity, Education,  Marital, HH_income, Work, HH_size,  Homeowner, Urbanicity, id, Extroversion, Agreeableness, Conscientiousness, Neuroticism, Openness, revolve_12m_factor,  revolve_1m_factor))
#
...
# Below, I choose cp to use for prunning (highest rel error below the dashed line)
(cp.choice = cc_adopt_syn_5var.tree$cptable[4, "CP"]) # for pruning 9 for long and 5 for a short tree
cc_adopt_syn_5var_prune = prune.rpart(cc_adopt_syn_5var.tree, cp=cp.choice)
#
prp(cc_adopt_syn_5var_prune, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA,  under = T, tweak = 1.1, varlen = 0, faclen = 0, Margin = 0.0, digits = -2, main = 'Credit card adoption: Classification tree')#faclet=0 avoids abbreviations, tweak for char size. I remove extra = 100 to remove percentage of observations.
# main = 'Credit card adoption: Classification tree (synthetic dataset)'
...
#
unbanked1.df = subset(i7.df, select = c(unbanked, Age,  Gender, Race, Ethnicity, Education,  Marital, HH_income, Work, HH_size,  Homeowner, Urbanicity, id, Extroversion, Agreeableness, Conscientiousness, Neuroticism, Openness, revolve_12m_factor,  revolve_1m_factor))
#
...
# Below, I choose cp to use for prunning (highest rel error below the dashed line)
(cp.choice = cc_adopt_syn_5var.tree$cptable[4, "CP"]) # for pruning 
unbanked_syn_5var_prune = prune.rpart(unbanked_syn_5var.tree, cp=cp.choice)
#
prp(unbanked_syn_5var_prune, type = 3, box.palette = "auto", legend.x=NA, legend.y=NA,  under = T, tweak = 1.1, varlen = 0, faclen = 0, Margin = 0.0, digits = -2, main = 'Bank account adoption: Classification tree')#faclet=0 avoids abbreviations, tweak for char size. I remove extra = 100 to remove percentage of observations.
# main = 'Bank account adoption: Classification tree (synthetic dataset)')
